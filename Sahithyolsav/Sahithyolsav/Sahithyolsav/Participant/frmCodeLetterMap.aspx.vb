...
Public Class frmCodeLetterMap
    ...
    Private Sub bindHeader()
        ...
        End If
        participantHeader.Text = "Code Letter [ " & struserLevel & " ]"
    End Sub
    ...
    Protected Sub imgEdit_Click(ByVal sender As Object, ByVal e As ImageClickEventArgs)
        ...
        Try
            ...
            txtSection.Text = gvrow.Cells(3).Text
            txtItem.Text = gvrow.Cells(4).Text
            txtChessNum.Text = gvrow.Cells(5).Text
            txtCodeLetter.Text = gvrow.Cells(6).Text
            hintParticipantListId.Value = lblintParticipantListId.Text
            hintItemId.Value = lblintItemId.Text
            hCodeLetterMapID.Value = gvCodeLtterMapdetails.DataKeys(gvrow.RowIndex).Value.ToString()
    ...
    End Sub
    Protected Sub btnCodeletterSave_Click(ByVal sender As Object, ByVal e As ImageClickEventArgs)
        Dim gRow As GridViewRow = Nothing
        Dim lblintParticipantListId As Label = Nothing
        Dim lblintItemId As Label = Nothing
        Dim txtCodeLetter As TextBox = Nothing
        Try
            btnUpdate.Visible = False
            btnSave.Visible = True
            Dim btndetails As ImageButton = TryCast(sender, ImageButton)
            ...
            hintItemId.Value = lblintItemId.Text
            hCodeLetterMapID.Value = gvCodeLtterMapdetails.DataKeys(gvrow.RowIndex).Value.ToString()
            txtCodeLetter = gRow.FindControl("txtCodeLetter")
            Dim arrin As New ArrayList
            arrin.Add(Convert.ToInt32(gvCodeLtterMapdetails.DataKeys(gvrow.RowIndex).Value.ToString()))
            arrin.Add(Convert.ToInt32(lblintParticipantListId.Text.ToString))
            arrin.Add(txtCodeLetter.Text.ToString)
            arrin.Add(DateTime.Now)
            arrin.Add(Convert.ToInt32(lblintItemId.Text))
            Participant.SaveCodeLetterMap(arrin)
            bindGrid()
        Catch ex As Exception
    ...
    Protected Sub btnSave_Click(ByVal sender As Object, ByVal e As System.Web.UI.ImageClickEventArgs) Handles btnSave.Click
        Dim arrin As New ArrayList
        arrin.Add(Convert.ToInt32(hCodeLetterMapID.Value.ToString))
        arrin.Add(Convert.ToInt32(hintParticipantListId.Value))
        arrin.Add(txtCodeLetter.Text)
        arrin.Add(DateTime.Now)
    ...
    Protected Sub ddlSearchItem_SelectedIndexChanged(ByVal sender As Object, ByVal e As EventArgs) Handles ddlSearchItem.SelectedIndexChanged
